...
use crate::security::audit::{AuditConfig, AuditEvent, AuditEventType, AuditLogger};
...
impl Default for SyscallAnomalyConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            strict_mode: true,
            max_denied_events_per_minute: 10,
            max_total_events_per_minute: 100,
            alert_on_unknown_syscall: true,
            baseline_syscalls: vec![
                "read".into(), "write".into(), "open".into(), "close".into(), "exit".into(),
                "accept".into(), "listen".into(), "fork".into(), "fcntl".into(), "set_tid_address".into(), "set_robust_list".into()
            ],
            log_path: "anomaly.log".into(),
...
impl SyscallAnomalyDetector {
    ...
    fn emit_alert(&self, alert: &SyscallAnomalyAlert, exit_code: Option<i32>) {
        println!(
            "syscall anomaly detected: kind={:?} command={} syscall={} denied_last_min={} total_last_min={} sample={}",
            alert.kind,
            alert.command,
            alert.syscall.as_deref().unwrap_or("-"),
            alert.denied_events_last_minute,
            alert.total_events_last_minute,
            alert.sample
        );

        if let Err(error) = self.append_log_line(alert) {
            println!("failed to append syscall anomaly log: {error}");
        }
...
fn truncate_sample(raw: &str) -> String {
    ...
    let mut idx = MAX_ALERT_SAMPLE_CHARS;
...
fn command_identity(command: &str) -> String {
    ...
    if lowered.len() <= 64 {
        ...
        let mut boundary = 64;
        while !lowered.is_char_boundary(boundary) && boundary > 0 {
            boundary -= 1;
        }
